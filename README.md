...
> [!IMPORTANT]
> This repository is the `github19155/mybot` fork. For its Linux server management modes, use
> [Linux host-admin / container-root](./docs/linux-install.md): **host-admin** runs the Agent as host
> root, while **container-root** runs it as UID 0 only inside Docker by default.
>
> The PyPI package and the `HKUDS/nanobot` installer links retained below are the **Upstream
> HKUDS/nanobot release** channels. They do not include this fork's mybot changes. To run this fork,
> install from `github19155/mybot` source or use the Linux guide above.

Pick **one** install method:

| Track | Install with | Update with | What runs |
|---|---|---|---|
| This fork source | editable `github19155/mybot` checkout | `git pull --ff-only` + editable dependency sync | Python, WebUI, and TUI from this fork checkout |
| Upstream HKUDS/nanobot release | upstream installer, `uv`, or pip | the same upstream package tool | the released upstream Python/WebUI/TUI package |

Prerequisites: Python 3.11 or newer. Git and [Bun](https://bun.sh/) are only needed for a source install. Published upstream packages include the WebUI and fetch a checksummed, version-matched TUI archive—with its licenses, notices, corresponding application source, source offer, and relinking instructions—on first use.
...
**Upstream HKUDS/nanobot release — one-command setup**

These commands install the upstream release, not this fork:
...
The upstream command installs or upgrades `nanobot-ai` from PyPI. On a fresh local desktop, it then starts `nanobot webui` so you can configure the first provider and model in **Settings → Models**. SSH, headless, existing-config, and older-release paths keep the terminal setup wizard. The installer avoids system-wide pip installs by using an active virtual environment, `uv`, `pipx`, or a managed venv under `~/.nanobot/venv`.

To preview that upstream installer without changing your environment, pass `--dry-run`.
...
**Upstream HKUDS/nanobot release with `uv`**

```bash
uv tool install nanobot-ai
```

**Upstream HKUDS/nanobot release from PyPI with pip**

```bash
python -m pip install nanobot-ai
```

If pip reports `externally-managed-environment`, use a virtual environment or another isolated package tool. Those package commands still install the upstream PyPI release, not this fork.

**Install this fork from source**

Clone this repository and install it in editable mode. Bun is required because the source checkout runs the matching TUI directly instead of downloading an older release binary.
...
After that, the normal commands are identical. `nanobot` runs the TUI from this checkout, and `nanobot webui` rebuilds stale frontend assets automatically. A later `git pull --ff-only` updates this fork's Python, TUI, and WebUI source together; rerun `python -m pip install -e .` when Python dependencies change. Contributors should also read [`CONTRIBUTING.md`](./CONTRIBUTING.md).
...
If `nanobot` is not on `PATH` after a fork source install, use the source environment directly, for example `.venv/bin/python -m nanobot ...` on macOS/Linux. Use `uv tool run --from nanobot-ai nanobot ...` or `pipx run --spec nanobot-ai nanobot ...` only when you intentionally installed the upstream PyPI release.
