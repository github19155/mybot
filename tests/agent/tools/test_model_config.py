...
def test_task_runtime_precedence_and_provider_routing(monkeypatch):
    config = _config()
    config.subagent_roles["coder"].model_preset = "worker"
    config.providers.anthropic.api_key = "anthropic-private"
    parent = LLMRuntime.capture(
        provider=Mock(generation=GenerationSettings()),
        model="parent-model",
        context_window_tokens=128_000,
    )
    snapshots = []

    def snapshot(config, *, preset=None, preset_name=None):
        resolved = preset or config.resolve_preset(preset_name)
        runtime = LLMRuntime.capture(
            provider=Mock(generation=GenerationSettings()),
            model=resolved.model,
            context_window_tokens=128_000,
        )
        snapshots.append((config.get_provider_name(resolved.model, preset=resolved), runtime))
        return runtime

    monkeypatch.setattr("nanobot.agent.model_management.build_provider_snapshot", snapshot)
    monkeypatch.setattr("nanobot.agent.model_management.runtime_from_provider_snapshot", lambda runtime: runtime)
    service = ModelManagement(config)
    assert service.resolve_task_runtime(parent, role="researcher") is parent
    assert service.resolve_task_runtime(parent, role="coder").model == "openai/gpt-worker"
    assert service.resolve_task_runtime(parent, role="coder", model="anthropic/claude-test").model == "anthropic/claude-test"
    assert snapshots[-1][0] == "anthropic"
    assert parent.model == "parent-model"
    with pytest.raises(ValueError, match="Unknown subagent role"):
        service.resolve_task_runtime(parent, role="reviewer")
    with pytest.raises(ValueError, match="Unknown model preset"):
        service.resolve_task_runtime(parent, role="coder", model_preset="missing")
