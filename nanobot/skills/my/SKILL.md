---
name: my
description: Inspect and optionally adjust the agent's runtime state. Use to check the current model or canonical model ID, context window and runtime limits, workspace and tool configuration, subagent status, and request routing metadata such as channel, chat ID, and sender ID; diagnose unavailable capabilities; change allowed runtime settings; or store temporary session scratchpad values.
---
...
| Switch model (also adjusts context window) | `my(action="set", key="model_id", value="<canonical-model-id>")` |
...
- `model_id` changes are scoped to the current session and take effect next turn; other settings are in-memory only, and scratchpad values persist across turns but not restarts
- Direct `model` writes are rejected as read-only. Direct `context_window_tokens` writes are instance-wide and disabled during active sessions — switch to a configured `model_id` instead.
- Protected params have type/range validation: `max_iterations` (1–100), `context_window_tokens` (4096–1M); `model_id` must be a non-empty string resolving to a configured `models` entry (unknown IDs are rejected)
