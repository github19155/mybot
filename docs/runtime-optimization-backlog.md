...
| 5 | P1 | deferred | Browser takeover capture efficiency | Human takeover polls a 1920x1080 screenshot frequently; the sidecar captures PNG by starting ImageMagick for each request. This adds CPU, process, memory-copy, and network overhead. | Defer the realtime takeover transport work until stronger server hardware is available; retain the current persistent-browser architecture for now. |
| 6 | P1 | in-progress | Durable subagent interruption state | Background Subagents are process-local tasks/statuses. Gateway restart can make a running task disappear with no durable record that it was interrupted. | Persist the existing Subagent lifecycle/status history in a small SQLite ledger, mark stale `queued`/`running` rows as `interrupted` on startup, retain terminal history for 30 days, and do not auto-resume work. |
...
- Item 5 is explicitly deferred until stronger server hardware is available; no Browser takeover transport change is made by item 6.
- Item 6 preserves the existing Main/WebUI Subagent status contract and only adds durable lifecycle/history storage plus restart interruption visibility.
