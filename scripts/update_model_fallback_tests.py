...
# Dedicated failover suite is obsolete by design.
Path("tests/agent/test_runner_fallback.py").unlink(missing_ok=True)

# Resolver API no longer accepts fallback policy switches.
...
s = read(p).replace('loader.assert_called_once_with(include_fallbacks=True)', 'loader.assert_called_once_with()')
write(p, s)

# Dream still chooses a preset before a request; it no longer needs a fallback-policy flag.
p = "tests/agent/test_dream_control_plane.py"
s = read(p).replace('        include_fallbacks=False,\n', '')
write(p, s)

# Replace the old main-vs-dream fallback-cache test with one-runtime cache semantics.
p = "tests/agent/test_runtime_resolution_integration.py"
s = read(p).replace('from nanobot.providers.fallback_provider import FallbackProvider\n', '')
write(p, s)
...
# Place it before the next parametrized subagent section.
...
# Onboarding no longer exposes fallback_models.
drop_tests("tests/agent/test_onboard_logic.py", tokens=("_handle_fallback_models_field", "fallback_models"))

# Provider signature test should only cover the selected route now.
drop_tests("tests/providers/test_custom_thinking_style.py", tokens=("fallbackModels",))

# Usage observer remains on provider retry attempts; remove only the deleted wrapper test.
